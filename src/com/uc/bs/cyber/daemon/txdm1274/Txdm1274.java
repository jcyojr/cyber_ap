...
public class Txdm1274 extends Codm_BaseDaemon{
	...
	public static void main(String[] args) {
		...
		try {
			...
			Txdm1274.setProcess("1274", "수기고지서 주민세 법인세할 수납내역등록(구청별)", "thr_1274");  /* 업무데몬 등록 */
	...
	/*
	 * 1.주민세 법인세할 소인파일을 읽어서 파싱한다.
	 * 2.등록 한다.(멀티트랜잭션 호출)
	 * */
	private void readSoinFiles(){
		
		log.info("=====================================================================");
		log.info("=" + this.getClass().getName()+ " readSoinFiles() [주민세 법인세할 소인파일 파싱] Start =");
		log.info("=====================================================================");
		...
		try{
			...
			for(int i =0 ; i < listf.length ; i++) {
				...
				if(listf[i].isFile()) {
					...
					if(listf[i].getName().lastIndexOf("SI22") != -1 && listf[i].getName().lastIndexOf(".OK") > 0) { /*주민세 종합소득할 파일만 읽어들인다.*/
						
						log.info("주민세 법인세할소인파일 = " + listf[i].getAbsolutePath());
						log.info("주민세 법인세할소인파일 = " + listf[i].getName().substring(0, listf[i].getName().lastIndexOf(".")));
						...
						log.info("주민세 법인세할 소인파일 파싱 시간 : " + CbUtil.formatTime(queryElapse2 - queryElapse1));
						
						txdm1274_Parse_Insert(); /*사이버DB에 입력*/
	...
	private ArrayList<MapForm> setFileReader(String FileName) {
		...
		if(OS.indexOf("AIX") != -1) {
			FileLen = 616 + 1;   /*데이터(616) + Line Feed(1) UNIX*/
		}else{
			FileLen = 616 + 2;   /*데이터(616) + Line Feed(2) WINDOWS*/
		}
	...
	private void txdm1274_Parse_Insert() {
		...
		log.info("=====================================================================");
		log.info("=" + this.getClass().getName()+ " txdm1274_Parse_Insert() [법인세할 소인파일 등록] Start =");
		log.info("=====================================================================");
		...
		try {
			...
			if (LevyCnt  >  0)   {
				...
				log.info("법인세할 소인파일등록 연계시간 : " + CbUtil.formatTime(elapseTime2 - elapseTime1));
				log.info("법인세할 소인파일등록 처리건수:: " + loop_cnt + ", 등록처리::" + insert_cnt + ", 기처리::" + update_cnt);
			}
