 ...
 *  클래스  ID : Txdm2432_process : 실 처리로직을 작성하는 클래스
 *  변경  이력 :
 * ------------------------------------------------------------------------
 *  작성자      	소속  		    일자            Tag           내용
 * ------------------------------------------------------------------------
...
public class Txdm2432_process extends Codm_BaseProcess implements Codm_interface {
	...
	private void mainTransProcess(){
		...
		try {
			...
			ststMap.setMap("LOG_DESC"  , "== END  가상계좌수납 실시간처리건수::" + loop_cnt + ", 수납처리::" + update_cnt + ", 기처리건::" + error_cnt);
	...
	public void transactionJob() {
		...
		if (alVSunapBusFineList.size()  >  0)   {
			...
			try{
				...
				daemonMap.setMap("DAEMON_NM" , "버스전용(NONTAX_TAB)가상계좌수납자료연계");
				...
				log.debug("버스전용(NONTAX_TAB)가상계좌수납자료연계 로그 등록 오류");
			}				
			/***********************************************************************************/
			...
			try{
				...
				log.debug("버스전용(NONTAX_TAB)가상계좌수납자료연계 대상이 없습니다. 등록 오류");
			}				
			/***********************************************************************************/
		...
		log.info("버스전용차로위반과태료 가상계좌수납 실시간처리건수::" + loop_cnt + ", 수납처리::" + update_cnt + ", 기처리건::" + error_cnt);
