 ...
 *  업  무  명 : 연계
 *  기  능  명 : 수기입력용사업장자정보수신 
 *  클래스  ID : txdm1172_process : 실 처리로직을 작성하는 클래스
 *  변경  이력 :
 * ------------------------------------------------------------------------
 *  작성자      	소속  		    일자            Tag           내용
 * ------------------------------------------------------------------------
 ...
 */
public class Txdm1172_process extends Codm_BaseProcess implements Codm_interface {
	...
	private void mainTransProcess(){
		...
		log.info("=====================================================================");
		log.info("=[수기입력용사업장자정보-구청 자료연계] Start =");
		log.info("=====================================================================");
		...
		try {
			...
			dataMap.setMap("PAGE_PER_CNT"   ,  2000);            /*페이지당 갯수*/
			
			do {
				
				int page = govService.getOneFieldInteger("txdm1172.txdm1172_select_count_page", dataMap);
	...
	private int txdm1172_JobProcess() {
		...
		log.info("=====================================================================");
		log.info("=" + this.getClass().getName()+ " txdm1172_JobProcess()[수기입력용사업장자정보-구청 자료연계] Start =");
		...
		try {
			...
			ArrayList<MapForm> alNidLevyList =  govService.queryForList("txdm1172.txdm1172_select_list", dataMap);
			...
			if (tot_size  >  0)   {
				...
				for ( rec_cnt = 0;  rec_cnt < tot_size;  rec_cnt++)   {
					...
					try{
						cyberService.queryForInsert("txdm1172.txdm1172_insert_tx1602_tb", gblNidLevyRows);
						insert_cnt++;
							...
							if (e instanceof DuplicateKeyException){	// 기본키가 중복되서 예외처리된경우 
								...
								try {
									cyberService.queryForInsert("txdm1172.txdm1172_update_tx1602_tb", gblNidLevyRows);
									update_cnt++;
...
//						remote_up_cnt+=	cyberService.queryForUpdate("txdm1172.txdm3141_update_state1", gblNidLevyRows);
					
					 }
				}
			}
				
				log.info("수기입력용사업장자정보수신 자료연계 건수 [" + rec_cnt + "] 등록건수 [" + insert_cnt +"] 수정건수 [" + update_cnt + "] 삭제건수 [" + delete_cnt + "] 물리적삭제 [" + p_del_cnt + "]");
