...
public class Tax004Service {
	...
	public MapForm sndService(MapForm sndForm) throws Exception {
		...
		try {
			...
			//System.out.println("종합소득 send msg [" + sndForm + "]");
			
			// 지방세 사업단으로 전송
			cMsg = new CreateMessages(sndForm);
						
			// getSOAPMessage : 전문 송신 & 수신 처리
			Element rcvElement = soap.getSOAPMessage(cMsg.getMessage(), "CM_TAX_53", "CM_TAX_53_125", SGG_COD);
			...
			if(message.equals("SVR01")){			//성공일때 확인 필요
				...
				taxVars = DocumentVars.getDocVars("140001");
				...
				// sndForm 필요한 값을 recvForm에 맵핑 
				recvForm.setMap("tax_gubun"      ,   "140001" );      						// 처리구분
