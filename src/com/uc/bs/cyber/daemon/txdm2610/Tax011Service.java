...
public class Tax011Service {
	...
	public MapForm sndService(MapForm sndForm) throws Exception {
		...
		try {
			...
			log.info("주민세 종업원분 send msg [" + sndForm + "]");
			
			// 지방세 사업단으로 전송
			cMsg = new CreateMessages_eswb5010(sndForm);
						
			// getSOAPMessage : 전문 송신 & 수신 처리
			Element rcvElement = soap.getSOAPMessage(cMsg.getMessage(), "CM_TAX_53", "CM_TAX_53_156", SGG_COD);
			...
			if(message.equals("SVR01")){			//성공일때 확인 필요
				...
				taxVars = DocumentVars.getDocVars("104011");
				...
				recvForm.setMap("tax_gubun"      ,   "104011" );      						// 처리구분
