 ...
 *  업  무  명 : 전자신고 
 *  기  능  명 : 지방소득세 양도소득  신고 등록
 *  클래스  ID : Tax005Service
 *  변경  이력 :
 * ------------------------------------------------------------------------
 *  작성자      	소속  		    일자            Tag           내용
 * ------------------------------------------------------------------------
...
public class Tax005Service {
	...
	public MapForm sndService(MapForm sndForm) throws Exception {
		...
		//System.out.println("sendform="+sndForm);

		SoapUtil soap = new SoapUtil();
		MapForm recvForm = new MapForm();
		
		CreateMessages cMsg = new CreateMessages(sndForm);
		
		String [][] taxVars  = cMsg.getTaxVars();

		try {
			
			//System.out.println("양도소득 send msg [" + sndForm + "]");
			
			// 지방세 사업단으로 전송
			cMsg = new CreateMessages(sndForm);
						
			// getSOAPMessage : 전문 송신 & 수신 처리
			Element rcvElement = soap.getSOAPMessage(cMsg.getMessage(), "CM_TAX_53", "CM_TAX_53_135", SGG_COD);
			...
			if(message.equals("SVR01")){			//성공일때 확인 필요
				...
				taxVars = DocumentVars.getDocVars("140002");
				...
				recvForm.setMap("tax_gubun"      ,   "140002" );
	            ...
	            recvForm.setMap("RCI_GRNO"       ,   CbUtil.convertDate(recvForm.getMap("GIRO_NO").toString()));        					  // 대장관리번호      
	            recvForm.setMap("B_TAX_NO"       ,   recvForm.getMap("TAX_NO"       ) );      // (31)납세번호
	            
	            
				/*
                recvForm.setMap("CUD_OPT"            ,   insertForm.get("CUD_OPT"      ) );      // 처리구분
                recvForm.setMap("DLQ_DIV"            ,   insertForm.get("DLQ_DIV"      ) );      // 미납체납구분
