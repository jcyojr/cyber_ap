 ...
 *  업  무  명 : 연계
 *  기  능  명 : 표준세외수입 수납자료연계
 *  클래스  ID : Txdm2413_process : 실 처리로직을 작성하는 클래스
 *  변경  이력 :
 * ------------------------------------------------------------------------
 *  작성자      	소속  		    일자            Tag           내용
 * ------------------------------------------------------------------------
...
public class Txdm2413_process extends Codm_BaseProcess implements Codm_interface {
	...
	private void mainTransProcess(){
		...
		log.info("=====================================================================");
		log.info("=[표준세외수입-[" + c_slf_org_nm + "] 수납자료연계] Start =");
		log.info("=====================================================================");
	...
	private int txdm2413_JobProcess() {
		...
		log.info("=====================================================================");
		log.info("=" + this.getClass().getName()+ " txdm2413_JobProcess()[표준세외수입-[" + c_slf_org_nm + "] 수납자료연계] Start =");
		...
		try {
		    ...
		    log.info("[" + c_slf_org_nm + "]세외수입수납연계자료 건수 = [" + tot_size + "]");
			...
			if (tot_size  >  0)   {
				...
				for ( rec_cnt = 0;  rec_cnt < tot_size;  rec_cnt++)   {
					...
					if(rec_cnt % 500 == 0) {
						log.info("[H][" + this.c_slf_org_nm + "][" + this.c_slf_org + "]세외수입수납자료처리중건수 = [" + rec_cnt + "]");
					}
					
				}
				
				log.info("[" + c_slf_org_nm + "]세외수입수납자료연계 건수 [" + rec_cnt + "] 등록건수 [" + insert_cnt +"] 수정건수 [" + update_cnt + "] 삭제건수 [" + delete_cnt + "] 물리적삭제 [" + p_del_cnt + "]");
				log.info("고지원장업데이트 ["+remote_up_cnt+"], 미처리 부과건수 [" + nul_cnt + "]");
			}

			elapseTime2 = System.currentTimeMillis();
			
			log.info("[" + c_slf_org_nm + "]세외수입 수납자료연계 시간("+tot_size+") : " + CbUtil.formatTime(elapseTime2 - elapseTime1));
