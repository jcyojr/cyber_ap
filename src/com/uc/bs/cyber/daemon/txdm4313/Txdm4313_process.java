 ...
 *  주시스템명 : 부산시청 사이버 지방세청
 *  업  무  명 : 연계
 *  기  능  명 : 상하수도 부가자료 연계(수시) 업무
 *  클래스  ID : Tx4313_process : 실 처리로직을 작성하는 클래스
 *  변경  이력 :
 * ------------------------------------------------------------------------
 *  작성자      	소속  		    일자            Tag           내용
 * ------------------------------------------------------------------------
 ...
 */
package com.uc.bs.cyber.daemon.txdm4313;
 ...
 *
 */
public class Txdm4313_process extends Codm_BaseProcess implements Codm_interface {
	...
	private void mainTransProcess(){
		...
		try {
			...
			do {
				...
				int page = govService.getOneFieldInteger("TXSV4313.select_count_page", dataMap);
				
				log.info("[상하수도 수시분 부과자료 (" + c_slf_org_nm + ")] PAGE_PER_CNT(" + dataMap.getMap("PAGE_PER_CNT") + ") 당 Page갯수 =" + page);
				...
				if(page == 0){
					...
					try{
						...
						daemonMap.setMap("DAEMON"    , "TXDM4313");
						daemonMap.setMap("DAEMON_NM" , "상하수도수시분(CYBER_PRTFEE_SUSI)부과자료연계");
						...
						log.debug("상하수도수시분(CYBER_PRTFEE_SUSI)부과자료연계 대상이 없습니다. 등록 오류");
					}				
					/***********************************************************************************/
				...
				if(govService.getOneFieldInteger("TXSV4313.select_count_page", dataMap) == 0) {
					break;
				}
	...
	/*자료 연계*/
	private int txsv4313_JobProcess() {
		
		log.info("=====================================================================");
		log.info("=" + this.getClass().getName()+ " txsv4313_JobProcess() [상하수도 고지 자료 연계] Start =");
		...
		try {
			...
			ArrayList<MapForm> alFixedLevyList =  govService.queryForList("TXSV4313.sudo_select_data", dataMap);
				...
				for ( int rec_cnt = 0;  rec_cnt < sudoCnt;  rec_cnt++)   {
					...
					if(trt_sp.equals("I")) {
						try{// STATE_FLAG 가 'I'일경우 insert
							cyberService.queryForList("TXSV4313.sudo_insert_data", mpTaxFixLevyList);
							insert_cnt++;
						}catch(Exception e){
							if (e instanceof DuplicateKeyException){
								//insert 할때 키 제약조건이 걸릴 때( 데이타가 있으므로 업데이트)
								try{
									cyberService.queryForList("TXSV4313.sudo_update_data", mpTaxFixLevyList);
									update_cnt++;
								}catch(Exception e1){
						...
						try{//STATE_FLAG가 'D'일 경우 삭제
							cyberService.queryForList("TXSV4313.new_sudo_delete_data", mpTaxFixLevyList);
							delete_cnt++;
						}catch(Exception e){
							log.error("오류데이터 = " + mpTaxFixLevyList.getMaps());
							log.error(e.getMessage());
					...
					try {
						...
						govService.queryForList("TXSV4313.sudo_update_srflag", mpTaxFixLevyList);
			...
			log.info("상하수도 수시분 고지자료 연계시간 : " + CbUtil.formatTime(elapseTime2 - elapseTime1));
			...
			try{
				...
				log.debug("상하수도수시분(CYBER_PRTFEE_SUSI)부과자료연계 로그 등록 오류");
			}				
			/***********************************************************************************/
