 ...
 *  클래스  ID : Txdm2474_process : 실 처리로직을 작성하는 클래스
 *  변경  이력 :
 * ------------------------------------------------------------------------
 *  작성자      	소속  		    일자            Tag           내용
 * ------------------------------------------------------------------------
...
public class Txdm2474_process extends Codm_BaseProcess implements Codm_interface {
	...
	private void mainTransProcess(){
		...
		try {
			...
			int CNT = 0;
			try{
				CNT = govService.getOneFieldInteger("TXDM2474.SELECT_ROAD_CNT", dataMap);
			...
			if(CNT>0){
				try{
					govService.queryForUpdate("TXDM2474.UPDATE_ROAD_START", dataMap);
				}catch (Exception sub_e){
				...
				try{
					govService.queryForUpdate("TXDM2474.UPDATE_ROAD_END", dataMap);
				}catch (Exception sub_e){
    ...
    /*주정차위반과태료...연계*/
	private int txdm2474_JobProcess() {
		
		log.info("=====================================================================");
		log.info("=" + this.getClass().getName()+ " txdm2474_JobProcess()[주정차위반과태료-[" + c_slf_org_nm + "] 부과자료연계] Start =");
		...
		try {
			...
			try{
				alNidLevyList =  govService.queryForList("TXDM2474.SELECT_ROAD_LIST", dataMap);
			}catch (Exception sub_e){
			...
			if (tot_size  >  0)   {
				...
				for ( rec_cnt = 0;  rec_cnt < tot_size;  rec_cnt++)   {
				    ...
				    if(TRN_YN.equals("1")&&WETAX_YN.equals("N")&&V_DEL_YN.equals("N")&&(EPAY_NO.equals(" ")||EPAY_NO.equals("0000000000000000000")||DUE_DT.equals(" ")||REG_NO.equals(" ")))TRN_YN="E";
					...
					if(TRN_YN.equals("1"))
				    {
						...
						if(WETAX_YN.equals("Y")){
							int TX2211_CNT_TAX_NO_WETAX = cyberService.getOneFieldInteger("TXDM2474.SELECT_TX2211_CNT_TAX_NO_WETAX", gblNidLevyRows);
							if(TX2211_CNT_TAX_NO_WETAX==0)
							{
								int TX2211_MAX_PAY_CNT = cyberService.getOneFieldInteger("TXDM2474.SELECT_TX2211_MAX_PAY_CNT", gblNidLevyRows);
								gblNidLevyRows.setMap("PAY_CNT",TX2211_MAX_PAY_CNT);
								String TX2112_OCR_BD = cyberService.getOneFieldString("TXDM2474.SELECT_TX2112_OCR_BD", gblNidLevyRows);
								gblNidLevyRows.setMap("OCR_BD",TX2112_OCR_BD);
								try{
									cyberService.queryForUpdate("TXDM2474.INSERT_TX2211", gblNidLevyRows);
								}catch (Exception sub_e){
									TRN_YN="E";
								...
								{
									try{
										cyberService.queryForDelete("TXDM2474.UPDATE_TX2112_DEL_YN_WETAX", gblNidLevyRows);
									}catch (Exception sub_e){
										TRN_YN="E";
										log.error("UPDATE_TX2112_DEL_YN_WETAX 오류 = " + gblNidLevyRows.getMaps());
										sub_e.printStackTrace();
									}
								}
							}
						...
						{
							//int TX2111_CNT_EPAY_NO = cyberService.getOneFieldInteger("TXDM2474.SELECT_TX2111_CNT_EPAY_NO", gblNidLevyRows);
							int TX2111_CNT_TAX_NO = cyberService.getOneFieldInteger("TXDM2474.SELECT_TX2111_CNT_TAX_NO", gblNidLevyRows);
							int TX2211_CNT_TAX_NO = cyberService.getOneFieldInteger("TXDM2474.SELECT_TX2211_CNT_TAX_NO", gblNidLevyRows);
							
							if(TX2111_CNT_TAX_NO==0){
								if(V_DEL_YN.equals("Y")){
									if(cyberService.getOneFieldInteger("TXDM2474.SELECT_TX2112_CNT_LEVY_DETAIL", gblNidLevyRows)>0){
										V_CUD ="L";
									}
								}else{
									V_CUD="I";
							...
							if(V_CUD.equals("I")){
								try{
									cyberService.queryForUpdate("TXDM2474.INSERT_TX2111", gblNidLevyRows);
								}catch (Exception sub_e){
									TRN_YN="E";
								...
								try{
									cyberService.queryForUpdate("TXDM2474.INSERT_TX2112", gblNidLevyRows);
								}catch (Exception sub_e){
									TRN_YN="E";
							...
							if(V_CUD.equals("U")){
								...
								try{	
									govService.queryForInsert("TXDM2474.INSERT_TX2115", gblNidLevyRows);
								}catch(Exception sub_e){
									TRN_YN="E";
									log.error("INSERT_TX2115 오류 = " + gblNidLevyRows.getMaps());
									sub_e.printStackTrace();
								}
								
								//차량번호,적발일시 변경시 가상계좌번호 삭제후 채번하여 재전송
								try{
									vir_no = cyberService.getOneFieldString("TXDM2474.getVirAccNoByTaxNoKey", gblNidLevyRows);	
								}catch (Exception sub_e){
									TRN_YN="E";
								...
								try{
									cyberService.queryForUpdate("TXDM2474.UPDATE_TX2111_TAX_NO", gblNidLevyRows);
								}catch (Exception sub_e){
									TRN_YN="E";
								...
								try{
									cyberService.queryForUpdate("TXDM2474.UPDATE_TX2112_TAX_NO", gblNidLevyRows);
								}catch (Exception sub_e){
									TRN_YN="E";
							...
							if(V_CUD.equals("D")){
								try{
									if (cyberService.queryForDelete("TXDM2474.UPDATE_TX2112_DEL_YN", gblNidLevyRows) > 0){
										delete_cnt++;
									}
								}catch (Exception sub_e){
									TRN_YN="E";
							...
							if(V_CUD.equals("L")){
								try{
									if (cyberService.queryForDelete("TXDM2474.UPDATE_TX2112_DEL_YN_LEVY_DETAIL", gblNidLevyRows) > 0){
										delete_cnt++;
									}
								}catch (Exception sub_e){
									TRN_YN="E";
									log.error("UPDATE_TX2112_DEL_YN_LEVY_DETAIL 오류 = " + gblNidLevyRows.getMaps());
									sub_e.printStackTrace();
								}
							}
						}
					...
					try{
						if (cyberService.queryForDelete("TXDM2474.UPDATE_TX2112_DEL_YN_EPAY_NO", gblNidLevyRows) > 0){
							delete_cnt++;
						}
					}catch (Exception sub_e){
						TRN_YN="E";
						log.error("UPDATE_TX2112_DEL_YN_EPAY_NO 오류 = " + gblNidLevyRows.getMaps());
						sub_e.printStackTrace();
					}
					...
					try{
						/*고지원장 업데이트*/
						remote_up_cnt +=govService.queryForUpdate("TXDM2474.UPDATE_ROAD_TRN_YN", gblNidLevyRows);
					}catch (Exception sub_e){
		    ...
		    log.error("TXDM2474 연계 오류 = " + gblNidLevyRows.getMaps());
