 ...
 *  주시스템명 : 부산시청 사이버 지방세청
 *  업  무  명 : 연계
 *  기  능  명 : 수기고지서 특별징수 수납내역전송 (구청별) 처리 업무
 *  클래스  ID : Txdm1271_process : 실 처리로직을 작성하는 클래스
 *  변경  이력 :
 * ------------------------------------------------------------------------
 *  작성자      	소속  		    일자            Tag           내용
 * ------------------------------------------------------------------------
 ...
 *
 */
public class Txdm1271_process extends Codm_BaseProcess implements Codm_interface {
	...
	private void mainTransProcess(){
		...
		try {
			...
			dataMap.setMap("rowcnt"         ,  10000);          /*최대 10000건씩*/
			dataMap.setMap("sido_cod"       ,  "26");
			dataMap.setMap("sgg_cod"        ,  c_slf_org);
			
			do {
				
				int page = cyberService.getOneFieldInteger("TXDM1271.tx1613_tb_select_page_cnt", dataMap);
				
				log.info("[지방세소득세 특별징수] PAGE_PER_CNT(" + dataMap.getMap("PAGE_PER_CNT") + ") 당 Page갯수 =" + page);
				...
				if(cyberService.getOneFieldInteger("TXDM1271.tx1613_tb_select_page_cnt", dataMap) == 0) {
					break;
				}
	...
	/*파싱한 자료를 사이버DB에 입력*/
	private void txdm1271_JobProcess() {
		
		log.info("=====================================================================");
		log.info("=" + this.getClass().getName()+ " txdm1271_JobProcess() [수기고지서 특별징수 수납내역전송] Start =");
		...
		try {
			...
			ArrayList<MapForm> alSpc_tx1613_List =  cyberService.queryForList("TXDM1271.tx1613_tb_select_list", dataMap);
			...
			if (LevyCnt  >  0)   {
				...
				for ( rec_cnt = 0;  rec_cnt < LevyCnt;  rec_cnt++)   {
					...
					try {
						...
						if(govService.getOneFieldInteger("TXDM1271.scyb503_select_cnt", mp_tx1613_tb_List) == 0 ) {
							...
							govService.queryForUpdate("TXDM1271.scyb503_insert_receipt", mp_tx1613_tb_List);
			...
			log.info("특별징수 수납내역전송(" + c_slf_org_nm + ") 자료연계 건수::" + rec_cnt + ", 등록건수::" + insert_cnt +", 기등록건수::" + update_cnt);
			log.info("특별징수 수납내역전송(" + c_slf_org_nm + ") 자료연계 시간::" + CbUtil.formatTime(elapseTime2 - elapseTime1));
