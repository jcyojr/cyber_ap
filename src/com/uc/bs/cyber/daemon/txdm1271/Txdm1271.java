 ...
 *  주시스템명 : 부산시 사이버지방세청
 *  업  무  명 : 연계
 *  기  능  명 : 수기고지서 특별징수 수납내역 등록(구청별) 연계 메인 
 *     
 *               5분단위로 동작...
 *  클래스  ID : Txdm1271
 *  사용  여부 : 
...
public class Txdm1271 extends Codm_BaseDaemon{
	...
	public static void main(String[] args) {
		...
		try {
			...
			Txdm1271.setProcess("1271", "수기고지서 특별징수 수납내역등록(구청별)", "thr_1271");  /* 업무데몬 등록 */
	...
	/*
	 * 1.특별징수 소인파일을 읽어서 파싱한다.
	 * 2.등록 한다.(멀티트랜잭션 호출)
	 * */
	private void readSoinFiles(){
		
		log.info("=====================================================================");
		log.info("=" + this.getClass().getName()+ " readSoinFiles() [특별징수 소인파일 파싱] Start =");
		log.info("=====================================================================");
		...
		try{
			...
			for(int i =0 ; i < listf.length ; i++) {
				...
				if(listf[i].isFile()) {
					...
					if(listf[i].getName().lastIndexOf("SI21") != -1 && listf[i].getName().lastIndexOf(".OK") > 0) { /*특별징수 파일만 읽어들인다.*/
						
						log.info("특별징수소인파일 = " + listf[i].getAbsolutePath());
						log.info("특별징수소인파일 = " + listf[i].getName());
						...
						log.info("소인파일(특별징수) 파싱 시간:" + CbUtil.formatTime(queryElapse2 - queryElapse1));
						
						txdm1271_Parse_Insert(); /*사이버DB에 입력*/
	...
	private ArrayList<MapForm> setFileReader(String FileName) {
		...
		if(OS.indexOf("AIX") != -1) {
			FileLen = 1198 + 1;   /*데이터(1198) + Line Feed(1) UNIX*/
		}else{
			FileLen = 1198 + 2;   /*데이터(1198) + Line Feed(2) WINDOWS*/
		}
	...
	private void txdm1271_Parse_Insert() {
		...
		log.info("=====================================================================");
		log.info("=" + this.getClass().getName()+ " txdm1271_Parse_Insert() [특별징수 소인파일 등록] Start =");
		log.info("=====================================================================");
		...
		try {
			...
			if (LevyCnt  >  0)   {
				...
				log.info("특별징수 소인파일등록 연계시간 : " + CbUtil.formatTime(elapseTime2 - elapseTime1));
				log.info("특별징수 소인파일등록 처리건수::" + loop_cnt + ", 등록처리::" + insert_cnt + ", 기처리::" + update_cnt);
			}
