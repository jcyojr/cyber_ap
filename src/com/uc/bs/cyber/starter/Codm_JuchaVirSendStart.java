...
public class Codm_JuchaVirSendStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_JuchaVirSendStart";
	...
	private static String  procId   = "0027";
	...
	public static void main(String[] args) {
		...
		Codm_JuchaVirSendStart coTransStart = new Codm_JuchaVirSendStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.juchavirsend.xml");
		
		log.info("=======================================");
		log.info("==      JuchaVirSendSystem Started       ==");
		log.info("=======================================");
		...
		try {
			...
			new Thread(coTransStart, "Main JuchaVirSend Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.juchavirsend.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main JuchaVirSend Daemon Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.juchavirsend.class" + i);
				...
				sb_key.append("cyber.juchavirsend.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.juchavirsend.title" + i);
