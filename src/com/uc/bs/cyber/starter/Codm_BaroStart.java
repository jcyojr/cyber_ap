...
public class Codm_BaroStart implements Runnable{
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_BaroStart";
	...
	private static String  procName = "바로결제 수납처리";
	private static String  procId   = "0019";
	...
	public static void main(String[] args) {
		...
		Codm_BaroStart coTransStart = new Codm_BaroStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.br.xml");
		...
		try {
			...
			new Thread(coTransStart, "Main baro Daemon Administrator").start();
	...
	public void run() {
		...
		*/
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.br.count")];
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.br.class" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.br.title" + i);
