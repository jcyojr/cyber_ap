...
public class Codm_BuvicarStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_BuvicarStart";
	...
	public static void main(String[] args) {
		...
		Codm_BuvicarStart coTransStart = new Codm_BuvicarStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.buvicar.xml");
		
		log.info("=======================================");
		log.info("==      BuvicarSystem Started       ==");
		log.info("=======================================");
		...
		try {
			...
			new Thread(coTransStart, "Main Buvicar Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.buvicar.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main buvicar Daemon Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.buvicar.class" + i);
				...
				sb_key.append("cyber.buvicar.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.buvicar.title" + i);
