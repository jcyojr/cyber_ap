...
public class Codm_TestStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_TestStart";
	...
	public static void main(String[] args) {
		...
		Codm_TestStart coTestStart = new Codm_TestStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTestStart, "log4j.test.xml");
		
		log.info("=======================================");
		log.info("==      CyberTestSystem Started       ==");
		log.info("=======================================");
		...
		try {
			...
			try {
				String[] xmls = XMLFileReader.getAllFilePathArray(Utils.getResourcePath(coTestStart, "/"), "SERVICE.XML");

				String strSrc = Utils.getResourcePath(coTestStart, "/") + "config/sqlmaps.xml";

				String strDest = Utils.getResourcePath(coTestStart, "/") + "config/sqlmapConfig.xml";

				XmlUtils.setXmlAttributes(coTestStart, strSrc, strDest, "sqlMapConfig", "sqlMap", "url", xmls);
			...
			coTestStart.setApp(context, "LT_etax");
			...
			new Thread(coTestStart, "Main Test Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.testdaemon.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main Test Daemon Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.testdaemon.class" + i);
				...
				sb_key.append("cyber.testdaemon.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.testdaemon.title" + i);
