...
public class Codm_SndStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_SndStart";
	...
	private static String  procName = "구청으로 전송데몬";
	private static String  procId   = "0005";
	...
	public static void main(String[] args) {
		...
		Codm_SndStart coTransStart = new Codm_SndStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.snd.xml");
		
		log.info("=======================================");
		log.info("==      SndSystem Started       ==");
		log.info("=======================================");
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.sndDm.count")];
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.sndDm.class" + i);
				...
				sb_key.append("cyber.sndDm.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.sndDm.title" + i);
