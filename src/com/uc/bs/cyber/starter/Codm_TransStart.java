...
public class Codm_TransStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_TransStart";
	...
	private static String  procId   = "0001";
	...
	public static void main(String[] args) {
		...
		Codm_TransStart coTransStart = new Codm_TransStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.trans.xml");
		
		log.info("=======================================");
		log.info("==      CyberTrans System Started    ==");
		log.info("=======================================");
		...
		try {
			...
			new Thread(coTransStart, "Main Trans Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.daemon.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main Trans Daemon Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.daemon.class" + i);
				...
				sb_key.append("cyber.daemon.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.daemon.title" + i);
