...
import com.uc.bs.cyber.daemon.txdm2512.Txdm2512;
import com.uc.bs.cyber.etax.net.RcvWorker;
import com.uc.bs.cyber.CbUtil;

public class Codm_DaemonBSVIRStart implements Runnable{
	
	private static IbatisService  singleService = null;
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_DaemonBSVIRStart";
	...
	private static String  procName = "BS 가상계좌 조회납부 데몬관리";
	private static String  procId   = "0024";
	private static int     loopTerm = 60;
	private static boolean isInitDaemonCstore = false;
	private static long    elapseTime = 0;
	
	public Codm_DaemonBSVIRStart() {
	...
	public static void main(String[] args) {
		...
		Codm_DaemonBSVIRStart coDaemonBsVirStart = new Codm_DaemonBSVIRStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coDaemonBsVirStart, "log4j.bv.xml");
		
		log.info("=======================================");
		log.info("==    Codm_DaemonBSVIRStart Started  ==");
		log.info("=======================================");
		...
		try {
			...
			try {
				String[] xmls = XMLFileReader.getAllFilePathArray(Utils.getResourcePath(coDaemonBsVirStart, "/"), "SERVICE.XML");

				String strSrc = Utils.getResourcePath(coDaemonBsVirStart, "/") + "config/sqlmaps.xml";

				String strDest = Utils.getResourcePath(coDaemonBsVirStart, "/") + "config/sqlmapConfig.xml";

				XmlUtils.setXmlAttributes(coDaemonBsVirStart, strSrc, strDest, "sqlMapConfig", "sqlMap", "url", xmls);
			...
			coDaemonBsVirStart.setApp(context, "LT_etax");
			
			new Thread(coDaemonBsVirStart, "Main Daemon(DP) Administrator").start();
	...
	public void run() {
		...
		try {
			...
			/*--------------------------------------------------*/
			/*1. 부산은행 소켓통신을 위한 Server.class 를 기동시킨다.*/
			/*--------------------------------------------------*/
			Txdm2512 txdm2512 = new Txdm2512(appContext);

			/*----------------------------------------------*/
			/*1. 업무용 데몬을 기동시킨다.                  */
			/*----------------------------------------------*/
			/* 업무 데몬에 Spring 컨텍스트를 주입해준다 */
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.bv.count")];
			
			daemonThread[0] = new Thread((Runnable) txdm2512);
			daemonThread[0].setName("2512");
	...
	private static void startDaemon(Thread[] dThread, ApplicationContext  context) throws Exception {
		...
		for(int i=0; i<dThread.length; i++) {
			...
			sb_key.append("cyber.bv.class" + i);
			...
			if(daemon instanceof RcvWorker) {
				...
				sb_key.append("cyber.bv.title" + i);
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main BsVir Daemon(DP) Administrator");           /*쓰레드 명*/
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			log.debug("== BS 가상계좌 조회납부 메인데몬 상태 등록중 오류발생 \n" + e.getStackTrace());
