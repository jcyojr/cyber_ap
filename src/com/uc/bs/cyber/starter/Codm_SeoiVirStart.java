...
public class Codm_SeoiVirStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_SeoiVirStart";
	...
	private static String  procName = "표준세외수입가상계좌연계";
	private static String  procId   = "0032";
	...
	public static void main(String[] args) {
		...
		Codm_SeoiVirStart coSeoiVirStart = new Codm_SeoiVirStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coSeoiVirStart, "log4j.seoivir.xml");
		
		log.info("=======================================");
		log.info("==    CyberSeoiVirSystem Started     ==");
		log.info("=======================================");
		...
		try {
			...
			try {
				String[] xmls = XMLFileReader.getAllFilePathArray(Utils.getResourcePath(coSeoiVirStart, "/"), "SERVICE.XML");

				String strSrc = Utils.getResourcePath(coSeoiVirStart, "/") + "config/sqlmaps.xml";

				String strDest = Utils.getResourcePath(coSeoiVirStart, "/") + "config/sqlmapConfig.xml";

				XmlUtils.setXmlAttributes(coSeoiVirStart, strSrc, strDest, "sqlMapConfig", "sqlMap", "url", xmls);
			...
			coSeoiVirStart.setApp(context, "LT_etax");
			...
			new Thread(coSeoiVirStart, "Main Seoi Vir Make Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.seoivir.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main Seoi vir make Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.seoivir.class" + i);
				...
				sb_key.append("cyber.seoivir.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.seoivir.title" + i);
