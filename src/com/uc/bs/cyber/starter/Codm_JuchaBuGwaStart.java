...
public class Codm_JuchaBuGwaStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_JuchaBuGwaStart";
	...
	private static String  procName = "사이버지방세청 연계데몬관리";
	private static String  procId   = "0028";
	private static int     loopTerm = 300;
	private static long    elapseTime = 0;
	private static boolean isInitDaemonStart = false;
	
	private static MapForm sysMap = new MapForm();

	public Codm_JuchaBuGwaStart() {
	...
	public static void main(String[] args) {
		...
		Codm_JuchaBuGwaStart coTransStart = new Codm_JuchaBuGwaStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.juchabugwa.xml");
		
		log.info("=======================================");
		log.info("==      JuchaBuGwaSystem Started       ==");
		log.info("=======================================");
		...
		try {
			...
			context = new ClassPathXmlApplicationContext("config/Sido-Spring-db.xml");
			...
			new Thread(coTransStart, "Main JuchaBuGwa Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			log.info("=============1. 업무용 데몬을 기동시킨다.==========================");
			
			/* 업무 데몬에 Spring 컨텍스트를 주입해준다 */
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.juchabugwa.count")];
			//daemonThread = new Thread[1];
			/* 이중화를 체크하기 위한 로직 */
			log.info("=============이중화를 체크하기 위한 로직.==========================" + CbUtil.getResourceInt("ApplicationResource", "cyber.juchabugwa.count"));
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main juchabugwa Daemon Administrator");         /*쓰레드 명*/
	...
	private static int getDaemonState(Thread[] dThread, ApplicationContext  context) {
		...
		log.info("procName 1111 " + procName);
		log.info("procId 1111 " + procId);
		try {
			/*
			 * 메인데몬 상태 확인
			 * */
			...
			if(procMap == null || procMap.isEmpty()) {
				...
				//lastTerm = ((BigDecimal) procMap.getMap("LAST_TERM")).longValue();
				lastTerm = 100;
			}
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.juchabugwa.class" + i);
				...
				sb_key.append("cyber.juchabugwa.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.juchabugwa.title" + i);
	...
	private static void startDaemon(Thread[] dThread, ApplicationContext  context) throws Exception {
		...
		for(int i=0; i<dThread.length; i++) {
			...
			log.info("==========  cyber.juchabugwa.class  ==========" + CbUtil.getResource("ApplicationResource", sb_key.toString()));
			...
			log.info("==========  proc_id  ==========" + CbUtil.getResource("ApplicationResource", sb_key.toString()));
