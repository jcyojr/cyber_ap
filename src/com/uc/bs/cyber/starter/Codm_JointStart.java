...
public class Codm_JointStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_JointStart";
	...
	private static String  procName = "사이버지방세청 연대납세자 자료송수신"; 
	private static String  procId   = "0015";
	...
	public static void main(String[] args) {
		...
		Codm_JointStart coTransStart = new Codm_JointStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.joint.xml");

		log.info("=======================================");
		log.info("==      JointSystem Started       ==");
		log.info("=======================================");
		...
		try {
			...
			new Thread(coTransStart, "Main Joint Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.joint.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main Joint Daemon Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.joint.class" + i);
				...
				sb_key.append("cyber.joint.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.joint.title" + i);
