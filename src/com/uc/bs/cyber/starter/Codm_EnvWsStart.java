...
public class Codm_EnvWsStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_EnvWsStart";
	...
	private static String  procName = "환경개선부담금 실시간수납";
	private static String  procId   = "0018";
	...
	public static void main(String[] args) {
		...
		Codm_EnvWsStart coTransStart = new Codm_EnvWsStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.envws.xml");
		
		log.info("===================================================================");
		log.info("==      간단 e 납부 환경개선부담금 실시간 수납전송 Started       ==");
		log.info("===================================================================");
		...
		try {
			...
			new Thread(coTransStart, "Main EnvWs Daemon Administrator").start();
	...
	public void run() {
		...
		*/
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.envws.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main Wsdl Daemon Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.envws.class" + i);
				...
				sb_key.append("cyber.envws.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.envws.title" + i);
