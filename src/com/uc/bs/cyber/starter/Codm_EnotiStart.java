...
public class Codm_EnotiStart implements Runnable{
		
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_EnotiStart";
	...
	private static String  procName = "상수도 전자고지 데몬";
	private static String  procId   = "0010";
	...
	public static void main(String[] args) {
		...
		Codm_EnotiStart coTransStart = new Codm_EnotiStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.enoti.xml");
	...
	public void run() {
		...
		*/
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.enoti.count")];
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.enoti.class" + i);
				...
				sb_key.append("cyber.enoti.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.enoti.title" + i);
