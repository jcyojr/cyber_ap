...
public class Codm_JugeojiStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_JugeojiStart";
	...
	private static String  procId   = "0031";
	...
	public static void main(String[] args) {
		...
		Codm_JugeojiStart coTransStart = new Codm_JugeojiStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.jugeoji.xml");
		
		log.info("=======================================");
		log.info("==      JugeojiSystem Started       ==");
		log.info("=======================================");
		...
		try {
			...
			context = new ClassPathXmlApplicationContext("config/Jugeoji-Spring-db.xml");
			
			coTransStart.setApp(context, "JUGEOJI");
			...
			new Thread(coTransStart, "Main Jugeoji Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.jugeoji.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main jugeoji Daemon Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.jugeoji.class" + i);
				...
				sb_key.append("cyber.jugeoji.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.jugeoji.title" + i);
