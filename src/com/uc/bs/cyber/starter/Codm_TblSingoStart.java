...
public class Codm_TblSingoStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_TblSingoStart";
	...
	private static String  procName          = "표준지방세자동차저당권전자신고";
	private static String  procId            = "0021";
	...
	public static void main(String[] args){
		...
		Codm_TblSingoStart coTblStart = new Codm_TblSingoStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTblStart, "log4j.tbl.xml");
		
		log.info("=======================================");
		log.info("==      Tbl Singo System Started     ==");
		log.info("=======================================");
		...
		try{
			...
			try{
				String[] xmls = XMLFileReader.getAllFilePathArray(Utils.getResourcePath(coTblStart, "/"), "SERVICE.XML");

				String strSrc = Utils.getResourcePath(coTblStart, "/") + "config/sqlmaps.xml";

				String strDest = Utils.getResourcePath(coTblStart, "/") + "config/sqlmapConfig.xml";

				XmlUtils.setXmlAttributes(coTblStart, strSrc, strDest, "sqlMapConfig", "sqlMap", "url", xmls);
			...
			coTblStart.setApp(context, "LT_etax");
			...
			new Thread(coTblStart, "Main Tbl Singo Daemon Administrator").start();
	...
	public void run(){
		...
		try{
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.tbl.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main tbl_singo Daemon Administrator");     /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception{
		...
		for(int i = 0; i<adThread.length; i++){
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.tbl.class" + i);
				...
				sb_key.append("cyber.tbl.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon){ // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.tbl.title" + i);
