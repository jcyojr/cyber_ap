...
public class Codm_TaxStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_TaxStart";
	...
	private static String  procName = "지방세 연계관리데몬";
	private static String  procId   = "0004";
	...
	public static void main(String[] args) {
		...
		Codm_TaxStart coTransStart = new Codm_TaxStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.tax.xml");
		
		log.info("=======================================");
		log.info("==      TaxSystem Started       ==");
		log.info("=======================================");
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.taxDm.count")];
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.taxDm.class" + i);
				...
				sb_key.append("cyber.taxDm.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.taxDm.title" + i);
