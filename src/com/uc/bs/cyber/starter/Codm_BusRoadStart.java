...
public class Codm_BusRoadStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_BusRoadStart";
	...
	private static String  procId   = "0033";
	...
	public static void main(String[] args) {
		...
		Codm_BusRoadStart coTransStart = new Codm_BusRoadStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.busroad.xml");
		
		log.info("=======================================");
		log.info("==      BusRoadSystem Started       ==");
		log.info("=======================================");
		...
		try {
			...
			context = new ClassPathXmlApplicationContext("config/Busroad-Spring-db.xml");
			...
			new Thread(coTransStart, "Main BusRoad Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.busroad.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main busroad Daemon Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.busroad.class" + i);
				...
				sb_key.append("cyber.busroad.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.busroad.title" + i);
