...
public class Codm_TrafficStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_TrafficStart";
	...
	private static String  procId   = "0030";
	...
	public static void main(String[] args) {
		...
		Codm_TrafficStart coTransStart = new Codm_TrafficStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coTransStart, "log4j.traffic.xml");
		
		log.info("=======================================");
		log.info("==      TrafficSystem Started       ==");
		log.info("=======================================");
		...
		try {
			...
			new Thread(coTransStart, "Main Traffic Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.traffic.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main traffic Daemon Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.traffic.class" + i);
				...
				sb_key.append("cyber.traffic.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.traffic.title" + i);
