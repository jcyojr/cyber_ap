...
public class Codm_SeoiStart implements Runnable{
	
	
	private static String ClsName = "com.uc.bs.cyber.starter.Codm_SeoiStart";
	...
	private static String  procName = "표준세외수입연계";
	private static String  procId   = "0020";
	private static int     loopTerm = 180;
	private static long    elapseTime = 0;
	private static boolean isInitDaemonStart = false;
	
	private static MapForm sysMap = new MapForm();

	public Codm_SeoiStart() {
	...
	public static void main(String[] args) {
		...
		Codm_SeoiStart coSeoiStart = new Codm_SeoiStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coSeoiStart, "log4j.seoi.xml");
		
		log.info("=======================================");
		log.info("==      CyberSeoiSystem Started       ==");
		log.info("=======================================");
		...
		try {
			...
			try {
				String[] xmls = XMLFileReader.getAllFilePathArray(Utils.getResourcePath(coSeoiStart, "/"), "SERVICE.XML");

				String strSrc = Utils.getResourcePath(coSeoiStart, "/") + "config/sqlmaps.xml";

				String strDest = Utils.getResourcePath(coSeoiStart, "/") + "config/sqlmapConfig.xml";

				XmlUtils.setXmlAttributes(coSeoiStart, strSrc, strDest, "sqlMapConfig", "sqlMap", "url", xmls);
			...
			coSeoiStart.setApp(context, "LT_etax");
			...
			new Thread(coSeoiStart, "Main Seoi Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.sedaemon.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main Seoi Daemon Administrator");         /*쓰레드 명*/
	...
	private static int getDaemonState(Thread[] dThread, ApplicationContext  context) {
		...
		try {
			...
			if(procMap == null || procMap.isEmpty()) {   /* 데이터가 조회되지 않았다면 한번도 실행된적 없는 데몬이다. */
				intvalue = 1;
			//}else if(!procMap.getMap("MANE_STAT").equals("1") || ((BigDecimal) procMap.getMap("LAST_TERM")).longValue() >= 900){  /* 프로세스 상태가 살아있지 않고 시간이 15분 지난데몬. */
			}else if(!procMap.getMap("MANE_STAT").equals("1")){  
				intvalue = 1;
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.sedaemon.class" + i);
				...
				sb_key.append("cyber.sedaemon.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.sedaemon.title" + i);
