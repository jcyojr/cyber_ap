...
public class Codm_BnonStart implements Runnable{

	private static String ClsName = "com.uc.bs.cyber.starter.Codm_BnonStart";
	...
	private static String  procName = "주정차위반과태료연계";
	private static String  procId   = "0023";
	...
	public static void main(String[] args) {
		...
		Codm_BnonStart coBnonStart = new Codm_BnonStart();
		
		/*Log4j 설정*/
		CbUtil.setupLog4jConfig(coBnonStart, "log4j.bnon.xml");
		
		log.info("=======================================");
		log.info("==      CyberBnonSystem Started       ==");
		log.info("=======================================");
		...
		try {
			...
			try {
				String[] xmls = XMLFileReader.getAllFilePathArray(Utils.getResourcePath(coBnonStart, "/"), "SERVICE.XML");

				String strSrc = Utils.getResourcePath(coBnonStart, "/") + "config/sqlmaps.xml";

				String strDest = Utils.getResourcePath(coBnonStart, "/") + "config/sqlmapConfig.xml";

				XmlUtils.setXmlAttributes(coBnonStart, strSrc, strDest, "sqlMapConfig", "sqlMap", "url", xmls);
			...
			context = new ClassPathXmlApplicationContext("config/Bnon-Spring-db.xml");
			
			coBnonStart.setApp(context, "LT_etax");
			...
			new Thread(coBnonStart, "Main Bnon Daemon Administrator").start();
	...
	public void run() {
		...
		try {
			...
			daemonThread = new Thread[CbUtil.getResourceInt("ApplicationResource", "cyber.bnonju.count")];
	...
	private static void setStateRegist(){
		...
		sysMap.setMap("THREAD_NM"    , "Main Bnon Daemon Administrator");         /*쓰레드 명*/
	...
	private static void aliveDaemon(Thread[] adThread, ApplicationContext  context) throws Exception {
		...
		for(int i = 0; i<adThread.length; i++) {
			...
			if(adThread[i] != null && !adThread[i].isAlive()){
				...
				sb_key.append("cyber.bnonju.class" + i);
				...
				sb_key.append("cyber.bnonju.name" + i);
				...
				if(daemon instanceof Codm_BaseDaemon) { // 일반데몬일 경우 데몬의 이름과 ID를 SET 해준다
					...
					sb_key.append("cyber.bnonju.title" + i);
