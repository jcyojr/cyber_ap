 ...
 *                2014.04.18           수정 
 *                기존 처리소스에 소스에 SNSU=1부분이 주석처리 되어 있었음(원인 불명확)
 *                그러한 사유로 인해 간단e납부의 취소요청이 이텍스납부(가상계좌등)을 취소 하는 경우가 발생(이때는 새올로 취소건을 보냄.)
 *                간단e납부로 납부된 건(SNSU=1)만 조회하여 취소할 수 있도록 주석을 제거하고 pay_cnt가  null일 경우 9999로 세팅하여 처리키로 함.
 *                
 *  
 */
		...
		try{
			...
			if (weCmdLevyList.size()  >  0)   {
				...
				for ( int rec_cnt = 0; rec_cnt < weCmdLevyList.size();  rec_cnt++ )   {
					...
					log.info("== CYB902001 수납취소" + (rec_cnt+1) +" ==(" + mfTotalLevyList.getMap("MSG") + ")");
					log.info("== CYB902001 수납취소" + (rec_cnt+1) +" ==(" + mfTotalLevyList.getMap("TRN_CD") + ")");
					
					String TRN_CD = mfTotalLevyList.getMap("TRN_CD").toString();
					
					//!.전자수납테이블을 삭제FLAG를 세우고 
					//!.환경개선부담금부과상세 테이블의 수납Flag를 ='0'으로 셋팅한다...
					...
					if (TRN_CD.equals("532001")) { /* 실시간납부건 취소 */
						...
						if(mf_2132_list == null || mf_2132_list.size() == 0) {
							...
							if (!((mf_2132_list.getMap("PAY_CNT").toString()).equals("9999"))) { //pay_cnt == 9999는 해당 SNSU코드의 수납건이 없다는 의미
							    /*4 환경개선부담금부과상세테이블의 수납FLAG를 셋팅 */
	                            ...
	                            log.info("환경개선부담금 취소처리 완료! [" + mfLog.getMap("EPAY_NO") + "]");
							}
							log.info("환경개선부담금 해당 간단e납부 수납건이 없으므로 패스]");
							retStr = "44120-000";
						}
						...
						log.error("============================================");
						log.error("== CYB902001 전문코드 에러 :  "+ mfTotalLevyList.getMap("TRN_CD"));
						log.error("============================================");		
						return "44120-201";
					}
				...
				log.info("============================================");
				log.info("== CYB902001 chkweTax 수납조회 오류!! ( 총 0건)   ");
				log.info("============================================");
			...
			e.printStackTrace();
			log.error("============================================");
			log.error("== CYB902001 chkweTax Exception(시스템) ");
			log.error("============================================");
			retStr =  "44120-201";
		}
