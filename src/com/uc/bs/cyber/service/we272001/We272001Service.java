 ...
 *  업  무  명 : 간단e납부(세외수입) 실시간납부결과통지
 *  기  능  명 : 위택스-사이버세청 
 *               업무처리
 *  클래스  ID : We272001Service
 *  변경  이력 :
 * ------------------------------------------------------------------------
 *  작성자     소속  		 일자      Tag   내용
 * ------------------------------------------------------------------------   
 * 소도우   유채널(주)  2011.04.24     %01%  신규작성
 * 임창섭   유채널(주)  2013.11.04     %02%  간단e납부관련(표준세외수입(기타세입금 제외), 환경개선부담금) 추가 
 */
package com.uc.bs.cyber.service.we272001;
...
public class We272001Service {
	 ...
	 * 생성자
	 */
	public We272001Service(ApplicationContext context) {
	...
	public String chkweTax(MapForm mf) throws Exception {
		...
		try{
			...
			ArrayList<MapForm>  weCmdLevyList  =  sqlService_cyber.queryForList("WE272001.SELECT_BNON_SEARCH_LIST", mf);
			...
			if (weCmdLevyList.size()  ==  1)   {
				...
				if(log.isDebugEnabled()){
					log.info("============================================");
					log.info("== CYB272001 chkweTax 부과조회 정상!! ( 총 1건)   ");
					log.info("============================================");
				}
                ...
                mf.setMap("PAY_CNT", sqlService_cyber.getOneFieldInteger("WE272001.TX2211_TB_MaxPayCnt", mfLevyList));
                
                log.debug(" 표준세외수입 MaxPayCnt = " + mf.getMap("PAY_CNT").toString());
				...
				int intLog = 0;
				if(sqlService_cyber.getOneFieldInteger("WE272001.TX2211_TB_WetaxCnt", mfLevyList)==0)
                intLog = sqlService_cyber.queryForUpdate("WE272001.INSERT_TX2211_TB_EPAY", mf);  /*표준세외수입*/
				...
				if(!(mfLevyList.getMap("SNTG").equals("1") || mfLevyList.getMap("SNTG").equals("2"))) {  /*이미수납이 되어 있는 경우임...(아마 부산은행 인터넷뱅킹에서) 제외*/
					...
					if(intLog > 0) {
                        ...
                        sqlService_cyber.queryForUpdate("WE272001.UPDATE_TX2112_TB_NAPBU_INFO", mfLevyList);
                        
                        log.info("표준세외수입 전자납부처리 완료! [" + mfLevyList.getMap("TAX_NO") + "]");
                    }
					
				} else {
					log.info("기 수납처리된 건입니다. 전자수납테이블에 이중납부여를 확인하세요! [" + mfLevyList.getMap("TAX_NO") + "]" );
...
//				
				...
				if(log.isDebugEnabled()){
					log.info("============================================");
					log.info("== CYB272001 chkweTax 부과조회 오류!! 전자납부번호(" + mf.getMap("EPAY_NO") + ")( 총 " + 0 + "건)   ");
					log.info("============================================");
				}
				...
				if(log.isDebugEnabled()){
					log.info("============================================");
					log.info("== CYB272001 chkweTax 부과조회 오류!! 전자납부번호(" + mf.getMap("EPAY_NO") + ")( 총 " + weCmdLevyList.size() + "건)   ");
					log.info("============================================");
				}
			...
			log.error("============================================");
			log.error("== CYB272001 chkweTax Exception(시스템) ");
			log.error("============================================");
