...
public class Egbt2610 {
	...
	public static void main(String[] args) {
		...
		if(args.length != 5) {
			...
			System.err.println(" 사용법 :: Egbt2610  [부과년도] [부과월] [과목코드] [고지구분(1:전자고지, 2:자동이체안내)] [납기일]");
		...
		Egbt2610 batch = new Egbt2610();
		
		MapForm paramMap = new MapForm();
		
		paramMap.setMap("NOTI_YY", args[0]);
		paramMap.setMap("NOTI_MM", args[1]);
		paramMap.setMap("KWA_CD", args[2]);
		paramMap.setMap("NOTI_GB", args[3]);
		...
		try {
			...
			CbUtil.setupLog4jConfig(batch, "log4j.tomcat.xml");
	...
	private void mainProcess() {
		...
		// 1. 전자고지순번을 가져온다
		paramMap.setMap("NOTI_SNO", sqlService.getOneFieldInteger("SELECT SF_CO_JOB_SEQNO('90') FROM DUAL"));
		
		// 2. 각 구청별 전자고지 자료를 구축한다
		Iterator<String> sgLoop = sgList.iterator();
		
		while(sgLoop.hasNext()) {

			/**
			 * 구청코드 파라메터 대입
			 */
			paramMap.setMap("SGG_COD", sgLoop.next());
			
			int insertCnt = sqlService.queryForUpdate("EGBT2610.INSERT_ME1151", paramMap);
			
			log.info("INSERT SGG=" + paramMap.getMap("SGG_COD") + ", CNT==" + insertCnt);
			
		}
		
		// 3. 전자고지 자료를 발송한다 (Egbt2611 에서 발송하도록 이관) :: 전자고지 순번을 넘겨줘야한다.....
		...
		*/
		...
		System.out.println("전자고지자료 구축완료  순번:::" + paramMap.getMap("NOTI_SNO"));
	...
	private void sendEnotify() throws Exception {
		...
		String taxNm = sqlService.getOneFieldString("EGBT2610.SELECT_CO3301_KWANAME", paramMap.getMap("KWA_CD"));
